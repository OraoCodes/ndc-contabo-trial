...
export default function EditIndicator() {
  ...
  const { data: indicator, isLoading, isError } = useQuery({
  ...
  useEffect(() => {
    if (indicator) {
      const s = (indicator.sector || "").toLowerCase();
      setSector(s === "water" || s === "waste" ? s : "");
      setThematicAreaId(
        indicator.thematic_area_id != null ? String(indicator.thematic_area_id) : ""
      );
      setTitle(indicator.title ?? "");
      setIndicatorText(indicator.indicator_text ?? "");
      setMaxScore(
        indicator.weight != null ? String(indicator.weight) : ""
      );
    }
  }, [indicator]);
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: {
      ...
      weight?: number;
    }) => updateIndicator(numericId!, payload),
    onSuccess: () => {
    ...
    onError: (err: unknown) => {
      toast({
        title: "Error",
        description: (err as Error)?.message ?? "Failed to update indicator",
        variant: "destructive",
      });
    },
  });
  ...
  const handleSubmit = () => {
    if (!validate() || !currentSector || !numericId) return;
  ...
  if (!numericId || isError || (indicator === null && !isLoading)) {
    return (
      <MainLayout>
